...
async function cargarCasos() {
 ...
 console.log(casos);
  ...
  for (let caso of casos) {
     ...
     if(caso.estado=="0"||caso.estado=="1"||caso.estado=="2"||caso.estado=="4" )
     {
        if(localStorage.admin=="true"){
                let botoncerrar = '<a href="#" onclick="cerrarCaso(' + caso.id + ')" class="btn btn-danger btn-circle btn-sm"><i class="fas fa-trash"></i></a>';

                let casoHtml = '<tr><td>' + botoncerrar + '</td><td>'+caso.id+'</td><td>'+caso.investigador+'</td><td>' + ESTADO + '</td><td>'+ caso.id_caso_relacionado +'</td><td>'+ caso.id_brecha + '</td><td>'+ caso.id_proyecto + '</td><td>'+ caso.empresa + '</td><td>'+ caso.tipo + '</td><td>'+ caso.id_irregularidad + '</td><td>'+ caso.objetivo + '</td><td>'+ caso.incidencia + '</td><td>'+ caso.dias + '</td><td>'+ caso.operandi + '</td><td>'+ caso.area + '</td><td>'+ caso.deteccion + '</td><td>'+ caso.diagnostico + '</td><td>'+ caso.acciones + '</td><td>'+ caso.conclusiones + '</td><td>'+ caso.observaciones + '</td><td>'+ caso.soporte + '</td></tr>';
...
async function cerrarCaso(id) {
  ...
  let request = await fetch('api/caso_cerrado/' + id, {
     method: 'GET',
     headers: getHeaders()
   });
